...
public static partial class CanonicalPersonRoleMatcher
{
    ...
    public static bool IsPersonRoleQuestion(string question)
    {
        ...
        var asksPerson = normalized.Contains("چه کسی", StringComparison.Ordinal)
            || normalized.Contains("کیست", StringComparison.Ordinal)
            || normalized.Contains("کیه", StringComparison.Ordinal)
            || normalized.Contains("نام", StringComparison.Ordinal);
        ...
        var organizationalUnitTerms=new[]{"واحد","نهاد","گروه","کمیته","سازمان","بخش","اداره","معاونت"};
        var asksInstitution=normalized.Contains("چه نهادی",StringComparison.Ordinal)
            ...
            || organizationalUnitTerms.Any(tokens.Contains);
