...
export default function Home(){
  ...
  const [operationDraft,setOperationDraft]=useState({contractId:"",type:1,title:"",dueDate:"",amount:"",currency:"IRR",reminderDaysBefore:7,description:""});
  ...
  const [effectiveTemplate,setEffectiveTemplate]=useState<ContractTemplate|null>(null);
  ...
  function newContract(){const base=contractBaseDocuments.find(x=>x.isActive);const status=contractStatuses.find(x=>x.isActive);setContractDraft({id:"",documentId:base?.documentId??"",baseDocumentProfileId:base?.id,statusDefinitionId:status?.id,statusName:status?.name,subject:"",status:1,currency:"IRR",partyCount:0,parties:[],contractGroupIds:[],updatedAtUtc:new Date().toISOString(),createdAtUtc:new Date().toISOString()});setContractOpen(true)}
  ...
  async function submitContract(){if(!contractDraft?.documentId||!contractDraft.subject.trim()||!contractDraft.primaryContractGroupId){setNotice("سند، موضوع و گروه اصلی قرارداد الزامی است.");return}try{await saveContract({...contractDraft,id:contractDraft.id||undefined});setContractOpen(false);setContractDraft(null);await refresh()}catch(error){setNotice(error instanceof Error?error.message:"ثبت قرارداد انجام نشد")}}
...
<span>یافته‌ها، شواهد صفحه‌محور و تصمیم‌های کارشناسی اینجا نمایش داده می‌شوند.</span>
  ...
  const sectionInfo:Record<string,{title:string;description:string;icon:IconName}>={documents:{title:"مدیریت اسناد",description:"جست‌وجو، نسخه‌بندی و مدیریت امن اسناد سازمان",icon:"file"},contracts:{title:"مدیریت قراردادها",description:"چرخه کامل قراردادها، طرفین، مبالغ و سررسیدها",icon:"contract"},workflow:{title:"گردش تأیید قرارداد",description:"تأیید مرحله‌ای حقوقی، فنی، مالی و مدیریتی",icon:"check"},operations:{title:"ریسک و عملیات قرارداد",description:"سررسید، تمدید، پرداخت، تضمین، اعلان و ارزیابی ریسک",icon:"clock"},management:{title:"داشبورد مدیریتی قراردادها",description:"وضعیت تأییدها، ریسک‌ها و عملیات پیش‌رو",icon:"chart"},comparison:{title:"تطبیق هوشمند اسناد",description:"اجرای تطبیق بر اساس گروه، قواعد و سند مرجع پویا",icon:"compare"},assistant:{title:"دستیار هوشمند نگارش",description:"تولید و تمدید قرارداد با مدل‌ها و منابع مجاز سازمان",icon:"sparkles"},reports:{title:"گزارش‌های مدیریتی",description:"گزارش‌های قابل ممیزی بر پایه داده‌های جاری",icon:"chart"},basicData:{title:"اطلاعات پایه",description:"تعریف گزینه‌های اختصاصی هر سازمان برای استفاده در قراردادها",icon:"settings"},access:{title:"کاربران و سطوح دسترسی",description:"مدیریت کاربر، نقش، دسترسی نقش و اعطا یا رد مستقیم دسترسی",icon:"shield"},settings:{title:"تنظیمات پویای سازمان",description:"مدل‌ها، ویژگی گروه‌ها و رفتار هر بخش در زمان اجرا",icon:"settings"}};
  async function loadP5(){try{const [w,o,m]=await Promise.all([listWorkflows(),listOperations(),getManagementDashboard()]);setWorkflows(w);setOperations(o);setManagement(m)}catch(error){setNotice(error instanceof Error?error.message:"اطلاعات عملیات قرارداد دریافت نشد")}}
  async function beginWorkflow(contractId:string){try{await startWorkflow({contractId});await loadP5();setNotice("گردش تأیید چهارمرحله‌ای آغاز شد.")}catch(error){setNotice(error instanceof Error?error.message:"گردش کار آغاز نشد")}}
  async function workflowDecision(id:string,decision:number){const comment=window.prompt("نظر یا توضیح تصمیم را وارد کنید")??"";try{await decideWorkflow(id,decision,comment);await loadP5()}catch(error){setNotice(error instanceof Error?error.message:"تصمیم ثبت نشد")}}
  async function addOperation(){if(!operationDraft.contractId||!operationDraft.title||!operationDraft.dueDate)return;try{await createOperation({...operationDraft,amount:operationDraft.amount?Number(operationDraft.amount):undefined});setOperationDraft({contractId:"",type:1,title:"",dueDate:"",amount:"",currency:"IRR",reminderDaysBefore:7,description:""});await loadP5()}catch(error){setNotice(error instanceof Error?error.message:"عملیات ثبت نشد")}}
  async function addRisk(contractId:string){const score=Number(window.prompt("امتیاز ریسک از صفر تا صد","50"));if(!Number.isFinite(score))return;const summary=window.prompt("جمع‌بندی ارزیابی ریسک")??"";try{await assessContractRisk({contractId,summary,items:[{code:"GENERAL",title:"ارزیابی تخصصی قرارداد",weight:1,score,note:summary}]});await loadP5();setNotice("ارزیابی ریسک ثبت شد.")}catch(error){setNotice(error instanceof Error?error.message:"ریسک ثبت نشد")}}
...
<Icon name="plus" size={16}/>ایجاد قرارداد</button>
      ...
      :activeSection==="workflow"?<div className="settings-grid"><article className="panel"><h2>شروع گردش تأیید</h2><p>گردش به‌ترتیب حقوقی، فنی، مالی و مدیریتی اجرا می‌شود.</p>{contracts.map(x=><div className="catalog-row" key={x.id}><span>{x.subject}</span><button onClick={()=>void beginWorkflow(x.id)}>شروع گردش</button></div>)}</article><article className="panel"><h2>گردش‌های جاری</h2>{workflows.map(w=><div className="catalog-row" key={w.id}><span>{w.subject} — مرحله {w.currentStageOrder} — وضعیت {w.status}</span>{w.status===1&&<><button onClick={()=>void workflowDecision(w.id,2)}>تأیید</button><button onClick={()=>void workflowDecision(w.id,3)}>درخواست اصلاح</button><button onClick={()=>void workflowDecision(w.id,4)}>رد</button></>}</div>)}</article></div>
      :activeSection==="operations"?<div className="settings-grid"><article className="panel"><h2>ثبت عملیات قرارداد</h2><div className="form-grid"><select value={operationDraft.contractId} onChange={e=>setOperationDraft({...operationDraft,contractId:e.target.value})}><option value="">انتخاب قرارداد</option>{contracts.map(x=><option key={x.id} value={x.id}>{x.subject}</option>)}</select><select value={operationDraft.type} onChange={e=>setOperationDraft({...operationDraft,type:Number(e.target.value)})}><option value={1}>سررسید</option><option value={2}>تمدید</option><option value={3}>پرداخت</option><option value={4}>تضمین</option><option value={5}>اعلان</option></select><input placeholder="عنوان" value={operationDraft.title} onChange={e=>setOperationDraft({...operationDraft,title:e.target.value})}/><input type="date" value={operationDraft.dueDate} onChange={e=>setOperationDraft({...operationDraft,dueDate:e.target.value})}/><input type="number" placeholder="مبلغ" value={operationDraft.amount} onChange={e=>setOperationDraft({...operationDraft,amount:e.target.value})}/></div><button className="primary-button compact" onClick={()=>void addOperation()}>ثبت عملیات</button></article><article className="panel"><h2>عملیات و ارزیابی ریسک</h2>{operations.map(x=><div className="catalog-row" key={x.id}><span>{x.subject} — {x.title} — {formatDate(x.dueDate)} — وضعیت {x.status}</span><button onClick={()=>void changeOperationStatus(x.id,2).then(loadP5)}>تکمیل</button><button onClick={()=>void addRisk(x.contractId)}>ارزیابی ریسک</button><button onClick={()=>void deleteOperation(x.id).then(loadP5)}>حذف</button></div>)}</article></div>
      :activeSection==="management"?<div className="settings-grid"><article className="panel"><h2>شاخص‌های مدیریتی</h2><div className="stats-grid"><div><strong>{management?.activeContracts??0}</strong><span>قرارداد فعال</span></div><div><strong>{management?.pendingApprovals??0}</strong><span>تأیید در انتظار</span></div><div><strong>{management?.overdueOperations??0}</strong><span>عملیات عقب‌افتاده</span></div><div><strong>{management?.upcomingOperations??0}</strong><span>رویداد ۳۰ روز آینده</span></div><div><strong>{management?.highRisks??0}</strong><span>ریسک بالا/بحرانی</span></div></div></article><article className="panel"><h2>رویدادهای پیش‌رو</h2>{management?.upcoming.map(x=><div className="catalog-row" key={x.id}><span>{x.subject} — {x.title} — {formatDate(x.dueDate)}</span></div>)}</article></div>
...
<label className="field full"><span>گروه‌های قرارداد</span><div className="multi-choice">{contractGroups.filter(x=>x.isActive).map(group=>{const checked=contractDraft.contractGroupIds.includes(group.id);return <label key={group.id}><input type="checkbox" checked={checked} onChange={e=>setContractDraft({...contractDraft,contractGroupIds:e.target.checked?[...contractDraft.contractGroupIds,group.id]:contractDraft.contractGroupIds.filter(id=>id!==group.id),primaryContractGroupId:e.target.checked?(contractDraft.primaryContractGroupId||group.id):(contractDraft.primaryContractGroupId===group.id?undefined:contractDraft.primaryContractGroupId)})}/><span>{group.name}</span></label>})}</div></label>
<label className="field"><span>گروه اصلی</span><select value={contractDraft.primaryContractGroupId??""} onChange={e=>setContractDraft({...contractDraft,primaryContractGroupId:e.target.value||undefined})}><option value="">انتخاب گروه اصلی</option>{contractGroups.filter(x=>contractDraft.contractGroupIds.includes(x.id)).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
<label className="field"><span>قالب مؤثر</span><input readOnly value={effectiveTemplate?`${effectiveTemplate.name} — نسخه ${effectiveTemplate.version}`:(contractDraft.primaryContractGroupId&&contractDraft.startDate?"گروه فاقد قالب معتبر است":"ابتدا گروه اصلی و تاریخ شروع را انتخاب کنید")}/></label>
<label className="field full">
