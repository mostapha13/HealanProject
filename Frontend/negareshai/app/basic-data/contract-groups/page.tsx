...
export default function ContractGroupsPage() {
  ...
  const load = useCallback(async (targetPage = page) => {
    ...
    } catch { setError("دریافت انواع قرارداد انجام نشد."); }
  ...
  async function submit(event: FormEvent) {
    ...
    } catch { setError("ذخیره‌سازی نوع قرارداد انجام نشد؛ نام باید در سازمان یکتا باشد."); }
  }
  ...
  async function remove(id: string) {
    if (!window.confirm("نوع قرارداد حذف نرم شود؟")) return;
  ...
  return <main className="app-shell" dir="rtl"><section className="page-content">
    <header className="page-header"><div><p className="eyebrow">اطلاعات پایه</p><h1>انواع قرارداد و مسیر Template</h1><p>نوع قرارداد برای انتخاب خودکار Template، یافتن سوابق مشابه، گردش‌کار و سطح دسترسی استفاده می‌شود.</p></div></header>
    <article className="panel"><h2>{editing ? "ویرایش نوع قرارداد" : "ثبت نوع قرارداد"}</h2>
      <form className="form-grid" onSubmit={submit}><label className="field"><span>نام نوع قرارداد *</span><input value={name} onChange={e => setName(e.target.value)} required /></label>
      ...
      <div><button className="primary-button" type="submit">{editing ? "ذخیره تغییرات" : "ثبت نوع قرارداد"}</button>{editing && <button className="ghost-button" type="button" onClick={reset}>انصراف</button>}</div></form>
    </article>
    <article className="panel"><h2>فهرست انواع قرارداد</h2>{error && <p className="error-text">{error}</p>}{loading ? <p>در حال دریافت اطلاعات…</p> : <>
      ...
      {!items.length && <p>هنوز نوع قراردادی ثبت نشده است.</p>}
