...
import {
  getNotCertaintyAfterDeadlineReport,
...
function ReportCertainty({ onAlert }: any) {
  ...
  useEffect(() => {
    getSymbolList('', 1);
    handleGetNotCertaintyAfterDeadlineReport(1);
  }, []);
  ...
  const handleGetNotCertaintyAfterDeadlineReport = (
    pageNo: number,
    instrumentId?: string
  ) => {
    const data = {
  ...
  const onChangePage = (pageNo: number) => {
    handleGetNotCertaintyAfterDeadlineReport(pageNo, instrumentId);
  };
  const onSearchClick = () => {
    handleGetNotCertaintyAfterDeadlineReport(
      1,
      instrumentId ? instrumentId : ''
    );
  };
...
export default withAlert(ReportCertainty);
