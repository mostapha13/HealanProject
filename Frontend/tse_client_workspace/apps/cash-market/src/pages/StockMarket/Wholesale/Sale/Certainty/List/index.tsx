...
const tableHeader: HeaderTypes[] = [
  ...
  {
    title: 'درخواست قطعیت معامله',
    dataIndex: 'instrumentName',
    key: 'instrumentName',
    className: 'col-span-2 !justify-center',
    render: (item: any, record: any) => (
      <a
        href={`stock/certainty-sell-wholesale-request?wholesaleInstrumentId=${record?.instrumentId}&wholesaleId=${record?.wholesaleId}&wholesaleOrderId=${record?.wholesaleOrderId}&wholesaleTypeId=${record?.wholesaleTypeId}`}
      >
        <Icon name="icon-Ghateeyat" classname="text-lg text-black" />
      </a>
    ),
  },
];
...
function CertaintyListSellWholesale({ onAlert }: any) {
  const PageSize = 10;
...
export default withAlert(CertaintyListSellWholesale);
