...
import {
  getWholeSale,
  getWholesaleSellerAttachTypeCertainty,
  getWholesaleSellerCertaintyByOrderId,
  saveWholesaleCertainty,
...
function CertaintyRequestSellWholesale({ onAlert }: any) {
  ...
  useEffect(() => {
    if (orderId) {
      ...
      handleGetWholesaleSellerAttachTypeCertainty(wholesaleTypeId);
    }
  }, []);
  ...
  const handleGetWholesaleSellerAttachTypeCertainty = (
  ...
  const handleGetWholesaleSellerCertaintyByOrderId = (orderId: string) => {
    ...
    getWholesaleSellerCertaintyByOrderId({
      ...
      onSuccess: (res) => {
        ...
        handleGetWholesaleSellerAttachTypeCertainty(res?.wholesaleTypeId);
      },
      onFail,
    });
  };
  ...
  const onSubmitDocumentClick = () => {
    ...
    if (requireFileUploadComplete === true) {
      const data = {
        ...
        wholesaleId: orderId ? editModeData?.wholesaleId : wholesaleId,
        wholesaleOrderId: orderId
          ? editModeData?.wholesaleOrderId
          : wholesaleOrderId,
        wholesaleInstrumentId: orderId
          ? editModeData?.instrumentId
          : wholesaleInstrumentId,
        wholesaleCertaintyFiles: uploadFileListItem,
      };
      saveWholesaleCertainty({
        data: data,
        onSuccess: (res) => {
      ...
      <div className="w-full grid grid-cols-10 border-lightGray border ">
        <div className=" items-start flex border-b-2 col-span-10  justify-start  border-lightGray  px-4">
          <span className=" p-2 font-bold ">
            بارگذاری مدارک قطعیت معامله فروش
          </span>
        </div>
        ...
        <div className="grid col-span-10 grid-cols-10 gap-4  justify-between mx-4 mt-4 border-2 border-lightGray p-4">
          <div className="2xl:col-span-2 xl:col-span-5 lg:col-span-5 md:col-span-5 col-span-2 mx-2 ">
            <NewSelect
              ...
              options={[{ name: '', id: '' }, ...wholesaleAttachTypeData]}
              onChange={(value: any) => {
                setState({
                  selectedDocumentType: value,
                  selectedDocumentTypeError: false,
                  selectedDocumentTypeName: wholesaleAttachTypeData.filter(
                    (item: any) => item?.id === value
                  )?.[0]?.name,
                  selectedDocumentTypeIsMultiple:
                    wholesaleAttachTypeData.filter(
                      (item: any) => item?.id === value
                    )?.[0]?.isMultiple,
                });
              }}
        ...
        <Collapse
          ...
          title="مدارک و مستندات قطعیت معامله"
          expanded={true}
        >
...
export default withAlert(CertaintyRequestSellWholesale);
