...
import {
  wholeSaleConfirmCertainty,
  wholeSaleRejectCertainty,
  getWholeSale,
  getWholesaleSellerCertaintyByOrderId,
  uploadWholeSaleCertaintyDocument,
  getWholeSaleCertaintyDocument,
  deleteWholeSaleCertaintyDocument,
  getWholeSaleCertaintyDocumentType,
  endWholeSaleCertainty,
...
const initialState = {
  ...
  certaintyWholeSaleDocumentList: [],
  documentId: '',
  typeId: '',
  typeIdName: '',
  typeIdError: false,
...
function CertaintyRequestDetailSellWholesale({ onAlert }: any) {
  ...
  const isDetailPage =
    window.location.pathname === '/stock/certainty-sell-wholesale-details'
      ? true
      : false;
  const isUploadDocPage =
    window.location.pathname === '/stock/certainty-sell-wholesale-upload-doc'
      ? true
      : false;
  const isFyiPage =
    window.location.pathname === '/stock/certainty-sell-wholesale-fyi'
      ? true
      : false;
  const typeData = [
    {
  ...
  useEffect(() => {
    if (orderId) {
      getWorkFlow();
      handleGetWholesaleSellerCertaintyByOrderId(orderId);
    }
  }, []);
  const handleGetWholeSale = (orderId: any) => {
    getWholeSale({
      ...
      onSuccess: (item) => {
        ...
        if (isUploadDocPage) {
          handleGetWholeSaleCertaintyDocumentType(isCondition);
        }
      },
      onFail,
    });
  ...
  const handleGetWholesaleSellerCertaintyByOrderId = (orderId: string) => {
    const data = {
      OrderId: orderId,
    };
    getWholesaleSellerCertaintyByOrderId({
      ...
      onSuccess: (res) => {
        setState({
          id: res?.id,
          certaintyDetailData: res,
        });
        setUploadFileListItem(res?.wholesaleSellerCertaintyFiles);
        handleGetWholeSale(res?.wholesaleOrderId);
        handleGetWholeSaleCertaintyDocument(res?.wholesaleId);
      },
      onFail,
    });
  };
  ...
  const handleGetWholeSaleCertaintyDocument = (wholesaleId: string) => {
    getWholeSaleCertaintyDocument({
      data: {
      ...
      onSuccess: (res) => {
        setState({
          certaintyWholeSaleDocumentList: res,
        });
      },
      onFail,
    });
  };
  const handleGetWholeSaleCertaintyDocumentType = (isCondition: boolean) => {
    const data = {
  ...
  const onSubmitDocumentClick = () => {
    if (typeId && documentTypeId && permitNo && permitDate && permitFile) {
      const data = {
        orderId: orderId,
        id: uploadDocEditMode
          ? documentId
          : '00000000-0000-0000-0000-000000000000',
        wholeSaleId: certaintyDetailData?.wholesaleId,
      ...
      uploadWholeSaleCertaintyDocument({
        data: data,
        onSuccess: (res) => {
          handleGetWholeSaleCertaintyDocument(certaintyDetailData?.wholesaleId);
          setState({
            uploadDocEditMode: false,
            documentTypeId: '',
            permitNo: '',
      ...
      <div className=" border-lightGray border ">
        ...
        <div className=" items-start flex border-b-2  justify-between bg-lightGray border-lightGray  px-4">
          <div className=" flex flex-row">
            ...
            <span className=" p-2  ">{certaintyDetailData?.workFlowName}</span>
          </div>
          <div className=" flex flex-row">
            <span className=" p-2 font-bold ">کد پیگیری :</span>
            <span className=" p-2  ">
              {certaintyDetailData?.trackingNumber}
            </span>
          </div>
          <div className=" flex flex-row">
            <span className=" p-2 font-bold ">وضعیت :</span>
            <span className=" p-2  ">
              {certaintyDetailData?.orderStatusName}
            </span>
          </div>
        </div>
        {changePage == 'request' && (
          <>
                ...
                <div className="grid col-span-10 grid-cols-12 gap-4  justify-between mx-4 mt-4 shadow-md p-4 ">
                  ...
                  <div className="col-span-12 my-10">
                    <Table
                      ...
                      dataSource={certaintyWholeSaleDocumentList}
            ...
            {certaintyDetailData?.message?.length > 0 && (
              <div className="grid col-span-10 grid-cols-12 gap-4  justify-between mx-4 mt-4">
                ...
                <div className=" col-span-12  pb-4">
                  <Table
                    data={certaintyDetailData?.message}
                    columns={columns}
...
export default withAlert(CertaintyRequestDetailSellWholesale);
