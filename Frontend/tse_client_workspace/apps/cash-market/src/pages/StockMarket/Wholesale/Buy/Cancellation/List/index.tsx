...
import { getWholesaleBuyByBroker } from 'apps/cash-market/src/Controller/StockMarket/WholeSale';
...
const tableHeader: HeaderTypes[] = [
  ...
  },
  {
    ...
    render: (item: any, record: any) => (
      <a
        href={`stock/cancel-request-buy-wholesale?wholesaleBuyInstrumentId=${record?.instrumentId}&wholesaleBuyId=${record?.wholesaleId}&wholesaleBuyOrderId=${record?.wholesaleBuyOrderId}`}
      >
...
function CanellationListBuyWholeSale({ onAlert }: any) {
  const PageSize = 10;
  ...
  useEffect(() => {
    getSymbolList('', 1);
    getWholesaleBuyByBrokerData(1, false);
  }, []);
  ...
  const onChangePage = (pageNo: number) => {
    getWholesaleBuyByBrokerData(pageNo, false);
    setState({ PageNumber: pageNo });
  };

  const getWholesaleBuyByBrokerData = (
    PageNumber?: any,
  ...
  return (
    <div className="border-2 border-lightGray w-full grid grid-cols-12 p-4 min-h-[500px]">
      <div className=" col-span-12 items-start flex border-b-2 border-lightGray">
        <span className=" p-2 font-bold">درخواست عودت 10% وجه</span>
      </div>
      ...
      <div className="flex justify-end  col-span-12">
        <Button
          ...
          onClick={() => getWholesaleBuyByBrokerData(1, true)}
        >
          حذف فیلتر
        </Button>
        <Button
          className="bg-blue text-white w-[115px]"
          onClick={() => getWholesaleBuyByBrokerData(1)}
        >
          فیلتر
        </Button>
      </div>
...
export default withAlert(CanellationListBuyWholeSale);
