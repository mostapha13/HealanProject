...
import {
  ...
  exportWordDebit,
  getDebitBondList,
  getInstrumentDebitList,
  getInstrumentInfoByName,
} from 'apps/cash-market/src/Controller/Instrument';
...
function Debit({ onAlert }: any) {
  ...
  const getList = () => {
    ...
    getDebitBondList({ data, onSuccess: onSuccessList, onFail });
  };
  ...
  const getSymbolList = (text: string, pageNo: number) => {
    ...
    getInstrumentDebitList({ data, onSuccess: onSuccessSymbolList, onFail });
  };
  ...
  const columns: HeaderTypes[] = [
    ...
    {
      title: 'تاریخ انتشار اوراق',
      dataIndex: 'debitBondPublishDate',
      key: 'debitBondPublishDate',
      className: 'col-span-1',
      render: (item: any) => <span>{convertDateToJalali(item)}</span>,
    },
    {
      title: 'تاریخ سررسید اوراق',
      dataIndex: 'debitBondDueDate',
      key: 'debitBondDueDate',
      className: 'col-span-2',
      render: (item: any) => <span>{convertDateToJalali(item)}</span>,
    },
    {
  ...
  const onDetail = (record: any) => {
    navigate(
      `/instrument/instrument-debit-details?id=${record.instrumentDebitId}`
    );
  };
  ...
  const onEdit = (record: any) => {
    navigate(`/instrument/new-instrument?tab=4&id=${record.instrumentDebitId}`);
  };

  const onCreateSome = (record: any): void => {
    navigate(
      `/instrument/new-instrument?tab=4&id=${record.instrumentDebitId}&isCopy=true`
    );
  };
  ...
  const exportFile = () => {
    ...
    exportToExcelDebit({ data, onSuccess: onSuccessExport, onFail });
  };
  const onSuccessExport = (res: any) => {
    downloadFile(res, 'instrumentDebit.xlsx');
  };
  const exportWord = (item: any) => {
    const data = {
      id: item?.instrumentDebitId,
    };
    exportWordDebit({ data, onSuccess: onSuccessExportWord, onFail });
  };
  const onSuccessExportWord = (res: any) => {
    downloadFile(res, 'instrumentDebit.docx');
  };
  return (
    <div>
