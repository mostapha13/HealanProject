...
import { convertDateToJalali, separator, downloadFile } from '@tse/tools';
import { HeaderTypes } from '@tse/types';
import { useEffect, useStates, useNavigate } from '@tse/utils';
import {
  getInstrumentTradeOptionList,
  removeInstrumentTradeOption,
  getInstrumentInfoByName,
  exportToExcelTradeOption,
  exportWordTradeOption,
  getInstrumentListTradeOption,
...
function TradeOption({ onAlert }: any) {
  ...
  const getList = () => {
    ...
    getInstrumentTradeOptionList({ data, onSuccess: onSuccessList, onFail });
  };
  ...
  const exportFile = () => {
    ...
    exportToExcelTradeOption({ data, onSuccess: onSuccessExport, onFail });
  };

  const onSuccessExport = (res: any) => {
    downloadFile(res, 'instrumentTradeOption.xlsx');
  };
  ...
  const onDetail = (record: any) => {
    navigate(
      `/instrument/instrument-tradeOption-details?id=${record.tradeOptionId}`
    );
  };
  ...
  const onRemove = (item: any): void => {
    ...
    removeInstrumentTradeOption({ data, onSuccess: getList, onFail });
  };

  const onEdit = (record: any) => {
    navigate(`/instrument/new-instrument?tab=2&id=${record.tradeOptionId}`);
  };

  const onCreateSome = (record: any): void => {
    navigate(
      `/instrument/new-instrument?tab=2&id=${record.tradeOptionId}&isCopy=true`
    );
  };
  ...
  const exportWord = (item: any) => {
    ...
    exportWordTradeOption({ data, onSuccess: onSuccessExportWord, onFail });
  };
  const onSuccessExportWord = (res: any) => {
    downloadFile(res, 'instrumentTradeOption.docx');
  };
  return (
    <div>
      ...
      <NestedTable
        ...
        columns={columns}
        childKey="tradeOptionInstruments"
