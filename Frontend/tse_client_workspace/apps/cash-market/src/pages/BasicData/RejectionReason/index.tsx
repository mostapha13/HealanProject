...
function RejectionReason({ onAlert }: any) {
  ...
  const submit = ({ reasonTitle, denyReasonId }: any) => {
    const data = {
      ...(denyReasonId && { denyReasonId }),
      reasonTitle,
    };
    saveDenyReason({ data, onSuccess: onSuccessSave, onFail });
  };
  ...
  return (
    <>
      <div className="shadow-[0_0px_4px_rgba(0,0,0,0.2)] rounded-sm p-3">
        <span className="font-bold">ثبت علت رد</span>
        <SimpleForm
...
export default withAlert(RejectionReason);
