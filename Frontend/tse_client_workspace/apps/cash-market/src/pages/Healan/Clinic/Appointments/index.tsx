...
function AppointmentsPage({ onAlert }: { onAlert: (msg: unknown) => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setPatients(p.items ?? []);
      setDoctors(d.items ?? []);
      setServices((s as PaginatedResponse<ServiceType>).items ?? []);
      setInsurances(Array.isArray(i) ? i : (i as { items?: InsuranceCompany[] }).items ?? []);
    }).catch(() => {});
  }, []);
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <button type="button" className="healan-btn healan-btn--primary" onClick={() => setShowForm(true)}>
            + پذیرش جدید
          </button>
        }
      />
          ...
          <div className="healan-card__body">
            <div className="healan-form-grid">
              ...
              <div className="healan-form-field">
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, primaryInsuranceCompanyId: e.target.value ? +e.target.value : null })}
                >
              ...
              <div className="healan-form-field">
                <label>تأیید بیمه پایه</label>
                <select
                  value={form.confirmPrimaryInsuranceCompany ? '1' : '0'}
                  onChange={(e) => setForm({ ...form, confirmPrimaryInsuranceCompany: e.target.value === '1' })}
                >
            ...
            <div style={{ marginTop: '1rem' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '0.5rem' }}>
                {services.map((s) => (
                  <label key={s.serviceTypeId} style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', cursor: 'pointer', fontSize: '0.85rem' }}>
                    <input
                      type="checkbox"
                    ...
                    <tr key={a.appointmentId}>
                      ...
                      <td>
                        <div className="healan-actions">
                          {!isPaid && inv && (
                            <button type="button" className="healan-btn healan-btn--primary healan-btn--sm" onClick={() => handlePay(a.appointmentId)}>
                              پرداخت
                            </button>
                          )}
                          <button type="button" className="healan-btn healan-btn--outline healan-btn--sm" onClick={() => navigate(`/healan/appointments/${a.appointmentId}`)}>
                            جزئیات
                          </button>
                        </div>
                      </td>
