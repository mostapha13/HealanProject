//API controller: DossierQuestionAnswerSession
//Component name: برگزاری جلسه پرسش و پاسخ توسط بورس تهران  

import withAlert from 'apps/cash-market/src/hoc/withAlert';
import {
  useStates,
  useEffect,
  useState,
  useNavigate,
  useSearchParams,
} from '@tse/utils';
import { SmartTable, Table } from '@tse/components/organism';
import { Button, TextField, Upload } from '@tse/components/atoms';
import { HeaderTypes } from '@tse/types';
import {
  modalFindApi,
  modalSaveApi,
  modalConfirmApi,
  modalRejectApi,
} from 'apps/cash-market/src/Controller/Listing/Modal';
import { Icon } from '@tse/components/atoms';
import { ListingModalComponent } from '../../atoms/ListingModalComponent';
import { DatePicker } from '@tse/components/molecules';
import { uploadFile, downloadFileApi } from 'apps/cash-market/src/Controller';
import { convertDateToJalali, downloadFile } from '@tse/tools';
import { DownloadOutlined } from '@mui/icons-material';
const initialState = {
  ListData: [],
  isShowChat: false,
  QAFileUpload: null,
  liveStreamFileUpload: null,
  fileUploadError: false,
  startDate: '',
  startDateError: false,
  endDate: '',
  endDateError: false,
  QAFileUploadError: false,
  liveStreamFileUploadError: false,
};

export function DossierFileQuestionAnswerSessionModal(props: any) {
  const { isOpen, modalData, onChangeState, onAlert, handleGetParentApi } =
    props;
  const [state, setState] = useStates<any>(initialState);
  const {
    ListData,
    isShowChat,
    QAText,
    QATextError,
    liveStreamText,
    liveStreamTextError,
    startDate,
    startDateError,
    endDate,
    endDateError,
  } = state;
  const [searchParams] = useSearchParams();
  const dossierId =
    searchParams.get('id') != null ? searchParams.get('id') : null;

  const tableHeader: HeaderTypes[] = [
    {
      title: 'تاریخ شروع',
      dataIndex: 'startDate',
      key: 'startDate',
      render: (item: any) => convertDateToJalali(item),
    },
    {
      title: 'تاریخ پایان',
      dataIndex: 'endDate',
      key: 'endDate',
      render: (item: any) => convertDateToJalali(item),
    },
    {
      title: 'لینک خبر',
      dataIndex: 'newsLink',
      key: 'newsLink',
      render: (item: any) => (
        <a href={item} target="_blank" rel="noreferrer">
          {item}
        </a>
      ),
    },
    {
      title: 'لینک پرسش و پاسخ',
      dataIndex: 'liveLink',
      key: 'liveLink',
      render: (item: any) => (
        <a href={item} target="_blank" rel="noreferrer">
          {item}
        </a>
      ),
    },
    ...(ListData?.accessAction
      ? [
          {
            title: 'اعمال وضعیت',
            dataIndex: 'action',
            key: 'action',
            render: (_: any, item: any) => {
              return (
                <div className="flex flex-row items-center flitems-center justify-center">
                  <Button
                    className="w-16 h-7  bg-dossierModalTableConfirmButton text-white ml-2"
                    onClick={() =>
                      onAcceptTable(item?.dossierDataQuestionAnswerSessionId)
                    }
                    disabled={item?.dossierStateTypeId != 'NotChecked'}
                  >
                    <Icon
                      name="icon-ok"
                      classname="text-white text-base cursor-pointer pt-1"
                    />
                    تایید
                  </Button>
                  <Button
                    className="h-7 bg-dossierModalTableRejectButton text-white p-2"
                    onClick={() =>
                      onRejectTable(item?.dossierDataQuestionAnswerSessionId)
                    }
                    disabled={item?.dossierStateTypeId != 'NotChecked'}
                  >
                    <Icon
                      name="icon-cancel"
                      classname="text-white text-lg cursor-pointer pt-1"
                    />
                    نیاز به اصلاح
                  </Button>
                </div>
              );
            },
          },
        ]
      : []),
    ...(ListData?.accessSignature
      ? [
          {
            title: 'امضای دیجیتال',
            dataIndex: 'attachment',
            key: 'attachment',
            render: (item: any, record: any) => <span></span>,
          },
        ]
      : []),
    {
      title: 'وضعیت',
      dataIndex: 'dossierStateTypeId',
      key: 'dossierStateTypeId',
      render: (_: any, item: any) => {
        return item?.dossierStateTypeId == 'Confirm' ? (
          <Icon
            name="icon-ok-circle"
            classname="text-dossierModalTableConfirmButton text-lg cursor-pointer"
          />
        ) : item?.dossierStateTypeId == 'Reject' ? (
          <Icon
            name="icon-cancel-circle"
            classname="text-dossierModalTableRejectButton text-lg cursor-pointer"
          />
        ) : (
          <Icon
            name="icon-details"
            classname="text-gray text-lg cursor-pointer"
          />
        );
      },
    },
  ];

  useEffect(() => {
    if (modalData != null && isOpen) {
      handleGetList();
    }
  }, [modalData]);

  const onAcceptTable = (id: any) => {
    const data = { dossierDataQuestionAnswerSessionId: id };
    modalConfirmApi({
      data,
      url: modalData?.confirmApiName,
      onSuccess: (res: any) => handleGetList(),
      onFail,
    });
  };

  const onRejectTable = (id: any) => {
    const data = { dossierDataQuestionAnswerSessionId: id };
    modalRejectApi({
      data,
      url: modalData?.rejectApiName,
      onSuccess: (res: any) => handleGetList(),
      onFail,
    });
  };

  const onFail = (error: any) => {
    onAlert(error);
  };
  const onAddRow = () => {
    if (startDate && endDate && QAText && liveStreamText) {
      const data = {
        dossierId,
        submenuId: modalData?.submenuId,
        startDate,
        endDate,
        newsLink: QAText,
        liveLink: liveStreamText,
      };
      modalSaveApi({
        url: modalData?.saveApiName,
        data,
        onSuccess: (res: any) => onSuccessAddRow(),
        onFail,
      });
    } else {
      !startDate && setState({ startDateError: true });
      !endDate && setState({ endDateError: true });
      !QAText && setState({ QATextError: true });
      !liveStreamText && setState({ liveStreamTextError: true });
    }
  };
  const onSuccessAddRow = () => {
    setState({
      startDate: '',
      endDate: '',
      QAText: '',
      liveStreamText: '',
    });
    handleGetList();
  };

  const handleGetList = () => {
    const dataQuery = { dossierId, submenuId: modalData?.submenuId };
    modalFindApi({
      url: modalData?.findApiName,
      data: dataQuery,
      onSuccess: (res: any) => onSuccessGetList(res),
      onFail,
    });
  };
  const onSuccessGetList = (res: any) => {
    setState({ ListData: res });
  };

  const onCloseModal = () => {
    onChangeState(modalData?.componentName, false);
    setState({
      startDate: '',
      startDateError: false,
      endDate: '',
      endDateError: false,
      QAText: '',
      liveStreamText: null,
      QATextError: false,
      liveStreamTextError: false,
    });
  };

  const onSuccessSubmit = () => {
    handleGetParentApi(modalData);
    onCloseModal();
  };

  const onChangeDate = (value: any, state: string, stateError: string) => {
    setState({ [state]: value, [stateError]: false });
  };

  return (
    <ListingModalComponent
      onAlert={onAlert}
      isOpen={isOpen}
      data={modalData}
      onCloseModal={onCloseModal}
      isShowChat={isShowChat}
      listData={ListData}
      onSuccessSubmit={onSuccessSubmit}
      lastValue={`${convertDateToJalali(
        ListData?.dossierDataQuestionAnswerSessionInfoResult?.[0]?.startDate
      )}`}
    >
      <div
        className={` ${
          isShowChat
            ? '2xl:col-span-8  xl:col-span-8 lg:col-span-12 md:col-span-12'
            : 'col-span-12'
        } grid grid-cols-12  items-center justify-center bg-white rounded-2xl p-6 `}
      >
        <div className="flex col-span-12 justify-end px-2 items-center ">
          {ListData?.dossierStateTypes?.map((item: any) => {
            return (
              <div className="bg-[#f0efef] rounded-full py-2 px-4 flex flex-row items-center">
                <div className="rounded-full w-[0.45rem] h-[0.45rem] border-gray bg-gray" />
                <span className="px-1">{item?.dossierStateTypeName}</span>
              </div>
            );
          })}
          <Icon
            name="icon-chat"
            classname=" text-lg cursor-pointer px-2"
            onClick={() => setState({ isShowChat: !isShowChat })}
          />
        </div>
        <div className="col-span-12 grid grid-cols-12 gap-4 py-8">
          <div className="2xl:col-span-6 xl:col-span-6 lg:col-span-6 md:col-span-12 sm:col-span-12">
            <DatePicker
              label="تاریخ شروع"
              value={startDate}
              onChange={(value: any) =>
                onChangeDate(value, 'startDate', 'startDateError')
              }
              onClearDate={() =>
                onChangeDate('', 'startDate', 'startDateError')
              }
              required
              error={startDateError}
            />
          </div>
          <div className="2xl:col-span-6 xl:col-span-6 lg:col-span-6 md:col-span-12 sm:col-span-12">
            <DatePicker
              label="تاریخ پایان"
              value={endDate}
              onChange={(value: any) =>
                onChangeDate(value, 'endDate', 'endDateError')
              }
              onClearDate={() => onChangeDate('', 'endDate', 'endDateError')}
              required
              error={endDateError}
            />
          </div>
          <TextField
            label="لینک خبر پرسش و پاسخ"
            className="2xl:col-span-6 xl:col-span-6 lg:col-span-8 md:col-span-12 sm:col-span-12"
            value={QAText}
            onChange={(value: any) =>
              setState({
                QAText: value,
                QATextError: false,
              })
            }
            required
            errorMessage={QATextError}
            // type="Text"
          />

          <TextField
            label="لینک پخش زنده"
            className="2xl:col-span-6 xl:col-span-6 lg:col-span-8 md:col-span-12 sm:col-span-12"
            value={liveStreamText}
            onChange={(value: any) =>
              setState({
                liveStreamText: value,
                liveStreamTextError: false,
              })
            }
            required
            errorMessage={liveStreamTextError}
            // type="Text"
          />
          <div className="col-span-12 flex justify-end my-4 items-start">
            <Button
              className="w-32 h-9 bg-listingModalRegisterButton text-white"
              onClick={onAddRow}
            >
              ثبت
            </Button>
          </div>
        </div>

        <div className="col-span-12 mt-32">
          <SmartTable
            columns={tableHeader}
            data={
              ListData?.dossierDataQuestionAnswerSessionInfoResult?.length > 0
                ? ListData?.dossierDataQuestionAnswerSessionInfoResult
                : []
            }
            totalPages={1}
            pageSize={100}
          />
        </div>
      </div>
    </ListingModalComponent>
  );
}
export default withAlert(DossierFileQuestionAnswerSessionModal);
