//API controller: DossierReceiptAdmission
//Component name: رسید پرداخت حق پذیرش
...
export function DossierFilePaymentReceiptModal(props: any) {
  ...
  const tableHeader: HeaderTypes[] = [
    {
      title: 'نوع درخواست',
      dataIndex: 'receiptAdmissionTypeName',
      key: 'receiptAdmissionTypeName',
    },
    {
      title: 'تاریخ',
              ...
              return (
                <div className="flex flex-row items-center flitems-center justify-center">
                  <Button
                    ...
                    onClick={() =>
                      onAcceptTable(item?.dossierDataReceiptAdmissionId)
                    }
                  ...
                  <Button
                    ...
                    onClick={() =>
                      onRejectTable(item?.dossierDataReceiptAdmissionId)
                    }
                    disabled={item?.dossierStateTypeId != 'NotChecked'}
                  >
                    <Icon
  ...
  const onSaveClick = () => {
    if (dateValue && fileValue) {
      const data = {
        ...
        receiptAdmissionTypeId: radioValue,
  ...
  const onSuccessSave = () => {
    setState({ dateValue: '', fileValue: null, radioValue: 'RequestLetter' });
    handleGetList();
  };
  const handleGetReciptType = () => {
    receiptAdmissionType({
      onSuccess: (res: any) => setState({ paymentReceiptType: res }),
      onFail,
    });
  };
  const onAcceptTable = (id: any) => {
    const data = { dossierDataReceiptAdmissionId: id };
    modalConfirmApi({
      data,
  ...
  return (
    <ListingModalComponent
      ...
      lastValue={
        ListData?.dossierReceiptAdmissionInfoResult?.[0]
          ?.receiptAdmissionTypeName
      }
    >
      <div
        className={` ${
      ...
      >
        ...
        <div className=" col-span-12 grid grid-cols-12 gap-4 items-center justify-center rounded-2xl my-2">
          <div className="col-span-12">
            ...
            >
              {paymentReceiptType?.map((item: any) => (
        ...
        <div className="col-span-12 mt-32">
          <SmartTable
            columns={tableHeader}
            data={
              ListData?.dossierReceiptAdmissionInfoResult?.length > 0
                ? ListData?.dossierReceiptAdmissionInfoResult
                : []
            }
            totalPages={1}
            pageSize={100}
...
export default withAlert(DossierFilePaymentReceiptModal);
