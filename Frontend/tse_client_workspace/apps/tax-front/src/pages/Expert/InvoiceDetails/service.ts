...
export async function getTransactionFee({
  ...
  try {
    const res = await request.get({
      baseUrl,
      url: `Invoice/GetTransactionFeeByPeriodIdManager/${periodId}/${indicatorNumber}`,
    });
    onSuccess(res);
  } catch (error) {
    onFail(error);
...
export async function getMessagesWorkFlow({
  onSuccess,
  onFail,
  orderId,
}: ResultType) {
  try {
    const res = await request.get({
      baseUrl,
      url: `Invoice/GetMessageOrderWorkFlowByOrderId`,
      options: {
        orderId: orderId,
      },
    });
...
export async function getWorkFlowData({
  ...
  try {
    const res = await request.get({
      baseUrl,
      url: `Invoice/OrderWorkFlowByOrderId`,
      options: {
        orderId: orderId,
      },
    });
