...
import type { ProvinceType } from '@tse/types';
...
export const IranMap = (props: IranMapType) => {
  ...
  function handleHoverProvince(province: ProvinceType) {
    setProvinceName(province.decription || province.name);
    onHover?.(province);
  }
      ...
      <div className={styles['container']}>
        <div className={styles['map']}>
          <div className="flex items-center justify-center pt-6">
            <section className="flex flex-row justify-between gap-x-6">
              <span className="text-base">راهنمای نقشه:</span>
              <div className="flex flex-row items-center gap-x-2">
                <span className="text-base">دریا</span>
                <div className="bg-[#187193] w-4 h-4 border-2" />
              </div>
              <div className="flex flex-row items-center gap-x-2">
                <span className="text-base">فعال</span>
                <div className="bg-white w-4 h-4 border-2" />
              </div>
              <div className="flex flex-row items-center gap-x-2">
                <span className="text-base">غیر فعال</span>
                <div className="bg-[#d6d6d6] w-4 h-4 border-2" />
              </div>
              <div className="flex flex-row items-center gap-x-2">
                <span className="text-base">استان معین</span>
                <div className="bg-[#fff1df] w-4 h-4 border-2" />
              </div>
            </section>
          </div>
          {/* <button
          ...
          >
            ...
            {provinces.map((province: ProvinceType) => (
              ...
              >
                <path
                  key={province.id}
                  className={province.decription ? styles['province-moin'] : ''}
            ...
            {provinces.map((province: ProvinceType) => {
              return (
                <text
                  className={`text-base select-none pointer-events-none ${province.className}`}
                  x={province.x}
                  y={province.y}
