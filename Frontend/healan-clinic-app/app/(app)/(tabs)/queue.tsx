...
const OPS: ClinicModuleId[] = ['queue', 'appointments', 'patients', 'booking-reservations', 'booking-schedules'];

export default function QueueTab() {
  const router = useRouter();
  const { home } = useAccess();
  const items = home.allServices.filter((m) => OPS.includes(m.id)).slice(0, 6);

  return (
    <AppScreen padded={false}>
      <SafeAreaView edges={['top']} style={styles.top}>
        <Text style={styles.title}>نوبت و پذیرش</Text>
        <Text style={styles.sub}>میانبرهای عملیاتی روز</Text>
      </SafeAreaView>
      <ScrollView contentContainerStyle={styles.grid}>
        {items.length === 0 ? (
          <EmptyBlock title="دسترسی نوبت ندارید" />
        ) : (
