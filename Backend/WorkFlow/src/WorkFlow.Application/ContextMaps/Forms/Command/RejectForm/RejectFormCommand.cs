...
{
    public class RejectFormCommand : IRequest<Unit>
    {
        public Guid OrderId { get; set; }
        public string PublicMessage { get; set; }
        public string PrivateMessage { get; set; }
    }
    public class RejectFormCommandHandler : IRequestHandler<RejectFormCommand, Unit>
    {
        ...
        public RejectFormCommandHandler(IApplicationDbContext applicationDbContext, IMediator mediator, ICurrentUserService currentUserService, IMapper mapper, IWorkFlowGuidService workFlowGuidService)
        {
        ...
        public async Task<Unit> Handle(RejectFormCommand request, CancellationToken cancellationToken)
        {
            ...
            var nextWorkFlowGuides = await _workFlowGuidService.GetNextWorkFlowGuidId(currentWorkFlow.WorkFlowGuideId, WeightId.Back);
            if (nextWorkFlowGuides == null)
                throw new BadRequestExceptions("Next Level Not Declared");

            var order = _applicationDbContext.Orders.Where(w => w.OrderId == currentWorkFlow.OrderId).Include(t => t.WorkFlowUser).FirstOrDefault();
            if (order == null)
            ...
            if (nextWorkFlowGuides != null)
            {
                ...
                {
                    _applicationDbContext.WorkFlowItems.Add(new WorkFlowItem()
                    {
                        OrderId = currentWorkFlow.OrderId,
