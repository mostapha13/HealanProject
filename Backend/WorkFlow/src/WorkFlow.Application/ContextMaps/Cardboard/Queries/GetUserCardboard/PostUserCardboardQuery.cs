...
{
    public class PostUserCardboardQuery : AbstractSearchRequest<PaginatedList<UserCardboardResponse>>
    {
    ...
    public class PostUserCardboardQueryHandler : IRequestHandler<PostUserCardboardQuery, PaginatedList<UserCardboardResponse>>
    {
        private readonly IApplicationDbContext _applicationDbContext;
        private readonly IMapper _mapper;
        private readonly ICurrentUserService _currentUserService;

        public PostUserCardboardQueryHandler(IApplicationDbContext applicationDbContext, IMapper mapper, ICurrentUserService currentUserService)
        {
            _applicationDbContext = applicationDbContext;
            _mapper = mapper;
            _currentUserService = currentUserService;
        }
        public async Task<PaginatedList<UserCardboardResponse>> Handle(PostUserCardboardQuery request, CancellationToken cancellationToken)
        {
            ...
            var query = from wfg in _applicationDbContext.WorkFlowGuides
                       ...
                       !order.IsDeleted &&
                        (
                        request.OrderIds == null || !request.OrderIds.Any() || request.OrderIds.Contains(order.OrderId)
                        )
                        &&
                        (
                        currenUser.FundId.HasValue && order.FundId == currenUser.FundId && currenUser.WorkFlowUserGroupId == receiverGroup.WorkFlowUserGroupId ||
                        !currenUser.FundId.HasValue && currenUser.WorkFlowUserGroupId == receiverGroup.WorkFlowUserGroupId
                        )
                        &&
                        ...
                        select new UserCardboardResponse
                        {
                            ...
                            ForwardClass = form.ForwardClass,
                            BackwardClass = form.BackwardClass,
                            HasObserved = wf.HasObserved,
                            ExtraInfo = order.ExtraInfo,
                            WorkFlowId = wf.WorkFlowItemId,
                            WorkFlowGuidId = wfg.WorkFlowGuideId,
                            FormId = form.FormId,
                            FormStateId = form.FormStateId
                        };
